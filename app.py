...
def index():
    ...
    if request.method == "GET":
        ...
        # Get information of share holding  and balances from data base
...
def buy():
    ...
    if request.method == "POST":
        ...
        if bal_min_purch <= 0:
             return apology("Insuficient Funds for Purchase", 403)
...
def quote():
    ...
    if request.method == "POST":
        ...
        if quote_symbol == None:
            return apology("No stock symbol submited", 400)
...
def register():
    ...
    if request.method == "POST":
        ...
        # Ensure confirmatio password was submitted
...
def sell():
    ...
    if request.method == "POST":
        ...
        # Check if valid number os shares to sell have been submitted by users
        if sell_shares < 1:
        ...
        # Update history page with transaction by user
        db.execute("INSERT INTO trn_history (trn_date, numb_shares, buy_sell, trn_price, trn_value, name, symbol) VALUES (?, ?, ?, ?, ?, ?, ?)", date, sell_shares, sell , price, amount, name, symbol)
